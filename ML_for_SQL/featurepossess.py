...
def generate(odir, wdir, label):                #odir:要打开的数据集的路径    wdir:生成特征向量的文件路径    lable:标志打开的是正常数据(0)还是恶意流量(1)
    ...
    with open(odir, 'rb') as f:                 #打开原始数据文件，并将其中的文本数据逐行读取到一个列表变量data中
        ...
        for line in data:                       #将每一行的数据读取到变量line中
            ...
            num_len = len(re.compile(r'\d').findall(line))                      #该行代码使用了re.compile()函数来编译正则表达式 \d，该正则表达式匹配所有的数字字符。接着，使用 findall() 方法查找所有匹配的结果，并将其作为列表返回。最后，使用 len() 函数计算列表的长度，即数字的数量，并将结果赋值给变量 num_len
            if len(line) != 0:
            ...
            #key_num=line.count('and')+line.count('or')+line.count('xor%20')+line.count('sysobjects%20')+line.count('version%20')+line.count('substr%20')+line.count('len%20')+line.count('substring%20')+line.count('exists%20')
