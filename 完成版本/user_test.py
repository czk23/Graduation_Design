...
def url_logistic_test(url, signal1, signal2):
    # logistic模型检测正常流量
    ...
    control.Menu.output1 += "以下是逻辑回归模型进行的预测报告" + '\n'
    control.Menu.output1 += "以下报告中1代表真，0代表假" + '\n'
    control.Menu.output1 += "预测为正常流量: %d" % pred_logistic_normal + '\n'
    control.Menu.output1 += "预测为sql注入攻击: %d" % pred_logistic_sql + '\n'
    control.Menu.output1 += "预测为xss攻击: %d" % pred_logistic_xss + '\n'
    control.Menu.output1 += "预测为目录遍历攻击: %d" % pred_logistic_traversal + '\n'
    control.Menu.output1 += "预测为crlf注入攻击: %d" % pred_logistic_crlf + '\n'

    no = -1
    if res.count(1) != 1:           # 正例的数量大于1则认为是其他种类恶意流量
        control.Menu.output1 += "系统判断该流量不属于上述五种中的任何一种，怀疑可能是其他种类恶意流量" + '\n'
    else:
        no = res.index(1)           # 找到唯一一个正例的序号
        control.Menu.output1 += "逻辑回归模型预测该流量属于%s" % my_map[no] + '\n'
    control.Menu.output1 += "*" * control.N + "\n"
...
def url_knn_test(url, signal1, signal2):
    # knn模型检测正常流量
    ...
    control.Menu.output1 += "*" * control.N + "\n"
    control.Menu.output1 += "以下是KNN模型进行的预测报告" + '\n'
    control.Menu.output1 += "以下报告中1代表真，0代表假" + '\n'
    control.Menu.output1 += "预测为正常流量: %d" % pred_knn_normal + '\n'
    control.Menu.output1 += "预测为sql注入攻击: %d" % pred_knn_sql + '\n'
    control.Menu.output1 += "预测为xss攻击: %d" % pred_knn_xss + '\n'
    control.Menu.output1 += "预测为目录遍历攻击: %d" % pred_knn_traversal + '\n'
    control.Menu.output1 += "预测为crlf注入攻击: %d" % pred_knn_crlf + '\n'
...
def csv_knn_test(csv_dir, signal1, signal2):
    # knn模型检测normal注入攻击
    ...
    control.Menu.output1 += "*" * control.N + "\n"
    control.Menu.output1 += "以下是KNN模型进行的检测报告" + '\n'
    control.Menu.output1 += "数据集一共包括%d条URL数据" % count + '\n'
    control.Menu.output1 += "正常流量数量为: %d, 占比%.1f%%" % (count_normal, (count_normal / count) * 100) + '\n'
    control.Menu.output1 += "sql注入攻击数量为: %d, 占比%.1f%%" % (count_sql, (count_sql / count) * 100) + '\n'
    control.Menu.output1 += "xss攻击数量为: %d, 占比%.1f%%" % (count_xss, (count_xss / count) * 100) + '\n'
    control.Menu.output1 += "目录遍历攻击数量为: %d, 占比%.1f%%" % (count_traversal, (count_traversal / count) * 100) + '\n'
    control.Menu.output1 += "crlf注入攻击数量为: %d, 占比%.1f%%" % (count_crlf, (count_crlf / count) * 100) + '\n'
    control.Menu.output1 += "其他种类恶意流量数量为: %d, 占比%.1f%%" % (count_other, (count_other / count) * 100) + '\n'
    control.Menu.output1 += "*" * control.N + "\n"
    control.Menu.output1 += '\n'
    signal1.emit(control.Menu.output1)


def csv_logistic_test(csv_dir, signal1, signal2):
    ...
    control.Menu.output1 += "*" * control.N + "\n"
    control.Menu.output1 += "以下是逻辑回归模型进行的检测报告" + '\n'
    control.Menu.output1 += "数据集一共包括%d条URL数据" % count + '\n'
    control.Menu.output1 += "正常流量数量为: %d, 占比%.1f%%" % (count_normal, (count_normal / count) * 100) + '\n'
    control.Menu.output1 += "sql注入攻击数量为: %d, 占比%.1f%%" % (count_sql, (count_sql / count) * 100) + '\n'
    control.Menu.output1 += "xss攻击数量为: %d, 占比%.1f%%" % (count_xss, (count_xss / count) * 100) + '\n'
...
def user_test(signal1, signal2):
    while(1):
        control.Menu.output2 += "*" * control.N + "\n"
        control.Menu.output2 += "请选择是进行单个URL流量检测或者csv文件流量检测" + '\n'
        control.Menu.output2 += "1.单个URL流量检测      2.csv文件流量检测      3.退出" + '\n'
        control.Menu.output2 += "*" * control.N + "\n"
        control.Menu.output2 += '\n'
        signal2.emit(control.Menu.output2)
        choose1 = control.input_queue.get()
        if int(choose1) == 1:
            control.Menu.output2 += "*" * control.N + "\n"
            control.Menu.output2 += "请输入要检测的URL" + '\n'
            control.Menu.output2 += "*" * control.N + "\n"
            signal2.emit(control.Menu.output2)
            url = control.input_queue.get()
            control.Menu.output2 += "*" * control.N + "\n"
            control.Menu.output2 += "输入的待检测URL为: %s" % url + '\n'
            control.Menu.output2 += "*" * control.N + "\n"
            control.Menu.output2 += '\n'
            signal2.emit(control.Menu.output2)
            res_knn = url_knn_test(url, signal1, signal2)
            res_logistic = url_logistic_test(url, signal1, signal2)
            if res_knn == -1 and res_logistic == -1:
                control.Menu.output1 += "综合上述两个模型，系统判断该流量不属于上述五种中的任何一种，怀疑可能是其他种类恶意流量" + '\n'
                control.Menu.output1 += '\n'
                signal1.emit(control.Menu.output1)
            elif res_knn == res_logistic:
                control.Menu.output1 += "综合上述两个模型，系统判断该流量属于%s" % my_map[res_knn] + '\n'
                control.Menu.output1 += '\n'
                signal1.emit(control.Menu.output1)
            else:
                control.Menu.output1 += "综合上述两个模型，系统判断该流量可能属于%s, 也有可能属于%s, 请用户自行判断" % (my_map[res_knn], my_map[res_logistic]) + '\n'
                control.Menu.output1 += '\n'
                signal1.emit(control.Menu.output1)

            control.Menu.output2 += "*" * control.N + "\n"
            control.Menu.output2 += "系统反馈调查表:" + '\n'
            control.Menu.output2 += "请告诉系统此次检测是否准确" + '\n'
            control.Menu.output2 += "0: 不准确   1: 准确" + '\n'
            control.Menu.output2 += "十分感谢您的反馈，系统会根据此次反馈进行自我完善" + '\n'
            control.Menu.output2 += "*" * control.N + "\n"
            control.Menu.output2 += '\n'
            signal2.emit(control.Menu.output2)
            feedback = control.input_queue.get()
            # 将用户输入的url送到训练集中
            ...
            control.Menu.output2 += "*" * control.N + "\n"
            control.Menu.output2 += "请输入要检测的csv文件路径" + '\n'
            control.Menu.output2 += "*" * control.N + "\n"
            signal2.emit(control.Menu.output2)
            csv_dir = control.input_queue.get()
            control.Menu.output2 += "*" * control.N + "\n"
            control.Menu.output2 += "输入的csv路径为: %s" % csv_dir + '\n'
            control.Menu.output2 += "*" * control.N + "\n"
            control.Menu.output2 += '\n'
            signal2.emit(control.Menu.output2)
            csv_knn_test(csv_dir, signal1, signal2)
            csv_logistic_test(csv_dir, signal1, signal2)
        elif int(choose1) == 3:
            break
        else:
            continue
